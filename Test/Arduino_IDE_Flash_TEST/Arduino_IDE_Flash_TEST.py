...
def load_flash_from_bin(Verbose=False):
    """
    Loads raw binary machine code (from avrdude dump) into the CPU flash array.
    Assumes CPU.flash is an array of 16-bit words.
    """
    file_path = "Test/Arduino_IDE_Flash_TEST/arduino_flash_dump.bin"
    try:
    ...
    # AVR instructions are 16-bit. Ensure we have an even number of bytes.
    if len(raw_data) % 2 != 0:
        raw_data += b'\xFF' # Pad with empty memory if odd
    ...
    for i in range(0, len(raw_data), 2):
        byteLSB = raw_data[i]
        byteMSB = raw_data[i+1]

        # Combine into a 16-bit word (AVR is Little-Endian)
        instruction_word = (byteMSB << 8) | byteLSB
        
        CPU.flash[memory_position] = instruction_word

        if Verbose:
            print(f"Flash: {hex(instruction_word)} \t Mem_pos: {memory_position} \t Index: {i}")

        memory_position += 1
        
    if Verbose:
        print(f"[+] Successfully loaded {len(raw_data)} bytes into flash memory.")
...
COMMANDS_LIST= ["Build_Hardware","Instruction_test","Arduino_Code_Test","list_commands","printState","runCycles","Load_program_to_memory","Flash_Memory_Dump","Emulate_bootloader"]
