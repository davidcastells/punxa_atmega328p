...
class TimerCounter0(py4hw.Logic): #8 Bit timer 
    ...
    def handle_FAST_PWM_mode(self):
        ...
        elif self.COM0A == 2: ## non-inverting 
            if self.OCR0A == self.TCNT0:
                self.OC0A_val = 0
                #Interrupts
                if self.OCIE0A == 1:
        ...
        #OC0B
        if self.COM0B == 1: ## Disabled 
            self.OC0B_val = 0
            if self.OCR0B == self.TCNT0:
                if self.OCIE0B == 1:
                    self.TIFR0 |= 0b100

        elif self.COM0B == 2: ## non-inverting 
            if self.OCR0B == self.TCNT0:
                self.OC0B_val = 0
                #Interrupts
                if self.OCIE0B == 1:
    ...
    def Other_modes_Increment(self):
        if self.direction == 'Increment':
            ...
            if self.TCNT0 >= self.TOP:
                ...
                elif self.opMode == 'FAST_PWM':

                    if self.COM0A == 2: ## clear    
                        self.OC0A_val = 1
                    elif self.COM0A == 3: ## SET
                        self.OC0A_val = 0


                    if self.COM0B == 2: ## clear   
                        self.OC0B_val = 1 
                    elif self.COM0B == 3: ## SET 
                        self.OC0B_val = 0
    ...
    def Memory_access(self):
        ...
        if ((self.ADDR == self.TCCR2A_addr_LS) and (self.port0.instype.get() == 1)):
            ...
            if (self.port0.read.get() == 1) and (self.port0.write.get() == 0):
                self.port0.read_data.prepare(self.TCCR2B & 0b00001111)
                self.port0.resp.prepare(1)
            ...
            if (self.port0.read.get() == 1) and (self.port0.write.get() == 0):
                self.port0.read_data.prepare(self.TIMSK2 & 0b00000111)
                self.port0.resp.prepare(1)
            elif (self.port0.read.get() == 0) and (self.port0.write.get() == 1):
            ...
            if (self.port0.read.get() == 1) and (self.port0.write.get() == 0):
                self.port0.read_data.prepare(self.TIFR2 & 0b00000111)
                self.port0.resp.prepare(1)
            elif (self.port0.read.get() == 0) and (self.port0.write.get() == 1):
    ...
    def update_prescaler(self):
        ...
        if self.CS == 0: # No clock Source
            ...
            if self.CS != self.prevCS:
                self.prevCS = self.CS
                self.prescalerCounter = 0
        elif self.CS == 6: # External clock on T2 pin. Clock on falling edge.
            if self.CS != self.prevCS:
                self.prevCS = self.CS
                self.prescalerCounter = 0
            if (self.PrevT2 == 1 and self.T2.get() == 0):
                self.prescaler = 1
                self.incrementEnable = True
            else:
                self.incrementEnable = False
            self.PrevT2 = self.T2.get()

        elif self.CS == 7: # External clock on T2 pin. Clock on rising edge.
            if self.CS != self.prevCS:
                self.prevCS = self.CS
                self.prescalerCounter = 0
            if (self.PrevT2 == 0 and self.T2.get() == 1):
                self.prescaler = 1
                self.incrementEnable = True
            else: 
                self.incrementEnable = False
        ...
        if self.direction == 'Increment':
            ...
            if self.TCNT2 >= self.TOP:
                if self.opMode == 'CTC':
                    ...
                    if self.COM2A == 2: ## clear    
                        self.OC2A_val = 1
                    elif self.COM2A == 3: ## SET
                        self.OC2A_val = 0


                    if self.COM2B == 2: ## clear   
                        self.OC2B_val = 1 
                    elif self.COM2B == 3: ## SET 
                        self.OC2B_val = 0
...
class TimerCounter1(py4hw.Logic): #16 Bit timer
    ...
    def handle_Phase_Correct_PWM_mode(self):
        ...
        if self.COM1A == 1: # at 0 0 OC0A is disconected 
            ...
            if (self.direction == 'Increment') and (self.TCNT1 == self.OCR1A):
                self.OC1A_val = 0

            elif (self.direction == 'Decrement') and (self.TCNT1 == self.OCR1A):
                self.OC1A_val = 1
        ...
        if self.COM1B == 1: 
            ...
            if (self.direction == 'Increment') and (self.OCR1B == self.TCNT1) :
                self.OC1B_val = 0

            elif (self.direction == 'Decrement') and (self.OCR1B == self.TCNT1):
                self.OC1B_val = 1

        elif self.COM1B == 3:
            if (self.direction == 'Increment') and (self.TCNT1 == self.OCR1B):
                self.OC1B_val = 1

            elif (self.direction == 'Decrement') and (self.TCNT1 == self.OCR1B):
                self.OC1B_val = 0

    def handle_CTC_mode(self):

        if self.WGM == 12 and self.TCNT1 == self.TOP:
            self.TIFR1 |= 0b100_000
    ...
    def handle_FAST_PWM_mode(self):
        ...
        if self.TCNT1 == self.TOP:
            self.TIFR1 |= 0b001 # Set TOV1
            if self.WGM == 14:
                self.TIFR1 |= 0b100000 #Set ICRF1
    ...
    def Other_modes_Increment(self):
        if self.direction == 'Increment':
            ...
            if self.TCNT1 > self.TOP:
                if 'CTC' in self.opMode:
                    self.TCNT1 = self.BOTTOM
                    if self.OCIE1A == 1: ## Interrupt
                        self.TIFR1 |= 0b010

                elif 'FAST_PWM' in self.opMode:
                    self.TCNT1 = self.BOTTOM

                    if self.COM1A == 2: # CLEAR
                        self.OC1A_val = 1
                    elif self.COM1A == 3: # SET
                        self.OC1A_val = 0

                    if self.COM1B == 2: # CLEAR
                        self.OC1B_val = 1
                    elif self.COM1B == 3: # SET
                        self.OC1B_val = 0
            ...
            if self.TCNT1 > 0xFFFF:
                self.TCNT1&= 0xFFFF
                if self.TOIE1 == 1:
                    self.TIFR1 |= 0b001 

            # Ensure registers always reflect the post-reset state
            self.TCNT1H = ((self.TCNT1>>8) & 0xFF)
            self.TCNT1L = self.TCNT1 & 0xFF
    ...
    def increment(self):
        ...
        if (self.prescaler != 0) and (self.prescaler == self.prescalerCounter): 
            self.prescalerCounter = 0
            # Incrementing and Decrementing
            if 'Phase_Correct' in self.opMode:
                self.Phase_Correct_PWM_Increment()
                #return   # skip determine_outputs at TOP turnaround
            else:  # NORMAL, CTC, FAST_PWM
                self.Other_modes_Increment()
